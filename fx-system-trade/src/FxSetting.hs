...
getLossCutRate fsd =
  ...
  in if Fsd.failProfit ls == 0 || Fsd.failProfitCount ls == 0
     then -100
     else - (Fsd.failProfit ls / (fromIntegral $ Fsd.failProfitCount ls)) * 10
...
getLearningTime fsd =
  ...
  in if Fsd.trTrade ls == 0
     then 60
     else let l = truncate $ getLearningTestTimes fsd * (((fromIntegral $ getTradeHoldTime fsd) * (fromIntegral $ Fsd.trTradeDate ls `div` Fsd.trTrade ls)) / 10) 
