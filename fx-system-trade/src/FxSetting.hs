...
getLossCutRate fsd =
  ...
  in if Fsd.failProfit ls == 0 || Fsd.trFail ls == 0
     ...
     else -(Fsd.failProfit ls / (fromIntegral $ Fsd.trFail ls)) * ((log :: (Double -> Double)) $ getLearningTestTimes fsd)
...
getProfitRate fsd =
  ...
  in if Fsd.successProfit ls == 0 || Fsd.trSuccess ls == 0
     then Gsd.initalProperty Gsd.gsd
     else (Fsd.successProfit ls / (fromIntegral $ Fsd.trSuccess ls)) * ((log :: (Double -> Double)) $ getLearningTestTimes fsd)

getLearningTime :: Fsd.FxSettingData -> Int
getLearningTime fsd =
  let ls = Fsd.learningSetting $ Fsd.fxSetting fsd
  in truncate $ getLearningTestTimes fsd * (if Fsd.trTrade ls == 0
                                            then 60
                                            else (fromIntegral $ Fsd.trTradeDate ls `div` Fsd.trTrade ls))

getLearningTestTime :: Fsd.FxSettingData -> Int
getLearningTestTime fsd =
  truncate $ fromIntegral (getLearningTime fsd) * ((log :: (Double -> Double)) $ getLearningTestTimes fsd)
...
updateFxSettingLog :: Int -> Double -> Fsd.FxSettingData -> Fsd.FxSettingData -> Bool -> Fsd.FxSettingData
...
createRandomFxAlgorithmSetting reset ix = do
  taAndR <- getRandomR(max 1 (Fad.algorithmAndRate ix - Gsd.taMargin Gsd.gsd), Fad.algorithmAndRate ix + truncate ((fromIntegral $ Gsd.taMargin Gsd.gsd) * Gsd.taAndRate Gsd.gsd))
...
createRandomFxTechnicalAnalysisSetting reset ix = do
  taAndR <- getRandomR(max 1 (Fad.treeAnaAndRate ix - Gsd.taMargin Gsd.gsd), Fad.treeAnaAndRate ix + truncate ((fromIntegral $ Gsd.taMargin Gsd.gsd) * Gsd.taAndRate Gsd.gsd))
