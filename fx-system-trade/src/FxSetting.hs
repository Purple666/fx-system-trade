...
getLearningTime fsd =
  ...
  in if Fsd.trTrade ls == 0
     then 60
     else let l = truncate $ getLearningTestTimes fsd * fromIntegral (getSimChartMax fsd + fromIntegral (Fsd.trTradeDate ls `div` Fsd.trTrade ls))
...
getLearningTestTimes fsd =
  (fromIntegral .  Fsd.learningTestTimes $ Fsd.learningSetting fsd) + 2
  -- (log :: (Double -> Double)) $ 
