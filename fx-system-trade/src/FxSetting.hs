...
getLossCutRate fsd =
  ...
  in if Fsd.failProfit ls == 0 || Fsd.trFail ls == 0
     ...
     else -(Fsd.failProfit ls / (fromIntegral $ Fsd.trFail ls)) * (sqrt $ getLearningTestTimes fsd)
...
getProfitRate fsd =
  ...
  in if Fsd.successProfit ls == 0 || Fsd.trSuccess ls == 0
     then Gsd.initalProperty Gsd.gsd
     else (Fsd.successProfit ls / (fromIntegral $ Fsd.trSuccess ls)) * (sqrt $ getLearningTestTimes fsd)

getLearningTime :: Fsd.FxSettingData -> Int
getLearningTime fsd =
  let ls = Fsd.learningSetting $ Fsd.fxSetting fsd
  in truncate $ (fromIntegral . Fsd.learningTestTimes . Fsd.learningSetting $ Fsd.fxSetting fsd) * (fromIntegral $ getTradeHoldTime fsd) +
     if Fsd.trTrade ls == 0
     then 60
...
getLearningTestTimes fsd =
  (log :: (Double -> Double)) $ ((fromIntegral . Fsd.learningTestTimes . Fsd.learningSetting $ Fsd.fxSetting fsd) + 0)
