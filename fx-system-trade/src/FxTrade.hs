...
-- Gsd.initalProperty Gsd.gsd / Gsd.quantityRate Gsd.gsd 
...
{-    
        ...
        | (Ftd.side td == Ftd.None || (Fs.getTradeHoldTime fsd < Fcd.no cd - tradeNo && Ftd.side td == Ftd.Sell)) &&                               evaluateProfitInc fto ftado = (chart, Ftd.Buy)                                                                                         | (Ftd.side td == Ftd.None || ( && Fs.getTradeHoldTime fsd < Fcd.no cd - tradeNo && Ftd.side td == Ftd.Buy)) &&                            evaluateProfitDec fto ftado = (chart, Ftd.Sell)                                                                                 
        | otherwise = (0, Ftd.None)   
