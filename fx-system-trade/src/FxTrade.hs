      ...
      td' = td { Ftd.chart     = cd
               ...
               , Ftd.alcOpen = if bt
                               then 
                                 ...
                                 Fad.zeroFxalgorithmListCount
                                 
               , Ftd.alcCloseProfit = if bt
                                      then
...
backTest latest l td fsd xcd = do
  ...
  td'' <- foldl (\a ctd -> do td' <- a
                              ...
                              return td3)
          (pure td) ctdl
  let fsd' = Fs.updateFxSettingData ctdl td td'' fsd
  if not latest
    then Fm.writeFxSettingData "backtest" fsd' -- $ Fs.updateFxSettingData ctdl td td'' fsd
    else return ()
