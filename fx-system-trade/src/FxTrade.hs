               ...
               then if Ftd.side td == Ftd.Buy &&
                       (forceSell ||
                        ((Fs.getSimChartMax fsd < Fcd.no cd - (Fcd.no $ Ftd.rate td) && 
                         0.1 < chart - rate && evaluateProfitDec ftcp ftadcp) ||
                        chart - rate < 0.1 && evaluateProfitDec ftcl ftadcl))
                    then (chart - rate, (chart / rate) - 1, Ftd.Buy)
                    else if Ftd.side td == Ftd.Sell &&
                            (forceSell || 
                             ((Fs.getSimChartMax fsd < Fcd.no cd - (Fcd.no $ Ftd.rate td) && 
                               0.1 < rate - chart && evaluateProfitInc ftcp ftadcp) ||
                               rate - chart < 0.1 && evaluateProfitInc ftcl ftadcl))
