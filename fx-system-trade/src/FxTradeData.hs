...
getEvaluationValue x =
  profit x * unrealizedPL x  / ((fromIntegral $ chartLength x) {- * tradeDateAve x -})
  -- (profit x * unrealizedPL x * (getWinRatePure x) ^ (2 :: Int)) / ((fromIntegral $ chartLength x) * tradeDateAve x)
