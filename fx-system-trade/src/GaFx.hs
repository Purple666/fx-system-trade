...
learningLoop c n p fsd = do
  ...
  fsds' <- (map (\x -> do let lt'   = Fs.getLearningTime     x
                          ...
                          let tdlt = map (\y -> Ft.learning (Ft.initFxTradeData Ftd.Backtest) $
                              ...
                              p    = 10000000 * (Ftd.profit tdl + getProfitList tdlt) /
...
learning n fsd = do
  tdlts <- (M.elems .
           ...
           (sequence . M.map (\(y, p, c) -> do let fsd' = fsd { Fsd.fxSetting = y }
                                               ...
                                               return (10000000 * (Ftd.profit tdl + getProfitList tdlt) *
...
backTestLoop latest n endN td fsd = do
  ...
  (fsd3, tdt) <- if latest
                 then Ft.backTest (Gsd.backtestLatestTime Gsd.gsd) lok td fsd2
                      <$> ((++) <$>
                      ...
                      in Ft.backTest (lt + ltt * Gsd.learningTestCount Gsd.gsd) lok td fsd2
                         <$> ((++) <$>
                              Fm.getChartListBack    (n - 1) (Fs.getPrepareTimeAll fsd2) 0 <*>
