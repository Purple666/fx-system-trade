...
class (Show a, Eq a, Ord a) => Ga a where
  ...
  learning x =
    setHash <$> (gaLoop 0 (Gsd.gaNum Gsd.gsd) =<< createLoop (Gsd.gaNum Gsd.gsd) x)
...
createLoop :: (Ga a) => Int -> LearningData a -> IO (LearningData a)
createLoop e x = do
...
gaLoop c e x = do
  ...
  if null x'
    then gaLoop c e =<< createLoop e x
    else if 10 < c && maximumScore x' == maximumScore x
