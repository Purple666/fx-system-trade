...
evaluateOne ctd fsd f1 forceSell td fs =
  let cd        = Fad.taChart ctd
      ...
      (position, open)
        | (Ftd.side td == Ftd.None || (Ta.getPrepareTimeAll fsd < tradeDate && Ftd.side td == Ftd.Sell)) && evaluateProfitInc fto ftado = (chart, Ftd.Buy)
        | (Ftd.side td == Ftd.None || (Ta.getPrepareTimeAll fsd < tradeDate && Ftd.side td == Ftd.Buy)) && evaluateProfitDec fto ftado = (chart, Ftd.Sell)
      ...
      (profits, close)
        ...
        | Ftd.side td == Ftd.Buy &&
          (forceSell || lcd < tradeDate ||
           (0 < chart - tradeRate && Ta.getPrepareTimeAll fsd < tradeDate && evaluateProfitDec ftcp ftadcp) ||
           (chart - tradeRate < 0 && Ta.getPrepareTimeAll fsd < tradeDate && evaluateProfitDec ftcl ftadcl)) = (chart - tradeRate, Ftd.Buy)
        | Ftd.side td == Ftd.Sell &&
          (forceSell || lcd < tradeDate ||
            (0 < tradeRate - chart && Ta.getPrepareTimeAll fsd < tradeDate && evaluateProfitInc ftcp ftadcp) ||
            (tradeRate - chart < 0 && Ta.getPrepareTimeAll fsd < tradeDate && evaluateProfitInc ftcl ftadcl)) = (tradeRate - chart, Ftd.Sell)
        | otherwise = (0, Ftd.None)
