...
evaluate ctd fsdi fsd f1 forceSell td =
  let cd        = Fad.taChart ctd
      ...
      lcd = if 60 * 24 * 5 * 4  < lt + ltt * Gsd.learningTestCount Gsd.gsd + Fs.getTradeHoldTime fsdi
            then 60 * 24 * 5 * 4 
            else lt + ltt * Gsd.learningTestCount Gsd.gsd + Fs.getTradeHoldTime fsdi
{-
      lcd = 60 * 24 * 5 * 4 * 3
-}
      unrealizedPL
...
qq        | Ftd.side td == Ftd.None && evaluateProfitInc fto ftado = (chart, Ftd.Buy)
...
{-
        ...
        | Ftd.side td == Ftd.Buy && (forceSell || lcd < tradeDate ||
                                     (Fs.getTradeHoldTime fsdi < tradeDate &&
