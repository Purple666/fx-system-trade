...
getEvaluationValue fsd td =
  ...
  then - (Fsd.getLogProfit fsd * Ftd.realizedPL td * Ftd.profit td * Ftd.getWinRatePure td ^ 10) / (fromIntegral $ Ftd.chartLength td)
  else   (Fsd.getLogProfit fsd * Ftd.realizedPL td * Ftd.profit td * Ftd.getWinRatePure td ^ 10) / (fromIntegral $ Ftd.chartLength td)
