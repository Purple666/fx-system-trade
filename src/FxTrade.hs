...
-}        
      ...
      td' = td { Ftd.chart     = cd
               ...
               , Ftd.realizedPL = if close /= Ftd.None
                                  then Ftd.realizedPL td + (fromIntegral $ Ftd.unit td) * (tradeRate + profits) / 25
                                  else Ftd.realizedPL td
               , Ftd.unrealizedPL = if Ftd.side td == Ftd.Buy
                                    then Ftd.realizedPL td + (fromIntegral $ Ftd.unit td) * (tradeRate + (chart - tradeRate)) / 25
                                    else if Ftd.side td == Ftd.Sell
                                         then Ftd.realizedPL td + (fromIntegral $ Ftd.unit td) * (tradeRate + (tradeRate - chart)) / 25
                                         else Ftd.realizedPL td
                 }
  in (open, close, td', fs'')
