...
gaLearningDataFromLog n fsd td = do
  ...
  fsl'' <- mapM (\(fs, (p, c)) -> do (ltt, fc) <- Ft.getChartLearning n fsd { Fsd.fxSetting = fs } td
...
updateFxSettingLog profits fsd =
  ...
  in fsd { Fsd.fxSettingLog = Fsd.minFxSettingDelete $
                              if M.member fs fsl
                              then if (fst $ fsl M.! fs) + profits < 0
                                   then M.delete fs fsl
                                   else M.adjust (\(p, c) -> (p + profits, c + 1)) fs fsl
                              else if 0 < profits
