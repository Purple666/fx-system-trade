...
createRandomFxTechnicalAnalysisSetting ix = do
  taAndR <- getRandomR(1, 1 + Fad.treeAnaAndRate ix + Gsd.treeAndRate Gsd.gsd)
  taOrR  <- getRandomR(1, 1 + Fad.treeAnaOrRate  ix + Gsd.treeOrRate  Gsd.gsd)
...
crossoverOrdFxAlMaSetting a b = do
  die <- R.replicateM 2 $ R.getRandomR (True, False)
  (a', b') <- crossoverOrd 5 -- (Gsd.taMargin Gsd.gsd)
